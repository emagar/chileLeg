...
for (i in 1:nrow(tmpSel)){
    message(sprintf("loop %s of %s", i, nrow(tmpSel)))
    #i <-which(tmp$bol=="1022-15") # debug
    #i <-1740 # debug
    sel <- which(tmp$bol==tmpSel[i,1]) # select urgencias tied to one boletín
    tmpBillUrg <- tmp[sel,] # isolate urgencia messages
    #head(tmpBillUrg) # debug
    ## if (tmpSel[i,2]==1){                            # cases with single urgencia 
    ##     tmp$chainN <- tmp$links <- 1  # cases with single urgencia 
    ##     next                                        # cases with single urgencia 
    ## }
    if (tmpSel[i,2]==1) next     # cases with single urgencia 
...
head(tmp[tmp$chainN==0,])


if (tmpSel[i,2]==1){                            # cases with single urgencia 
        tmpBillUrg$chainN <- tmpBillUrg$links <- 1  # cases with single urgencia 

#
# rename elements
tmp$chain <- tmp$type <- NULL #; colnames(tmp)[grep("type", colnames(tmp))] <- "typOrig"
# fine-tune typeN: urg went all the way to deadline 1.0, 2,0, 3.0; deadline modified 1.4, 2.4, 3.4; retired 1.5, 2.5, 3.5; modified, then retired 1.45, 2.45, 3.45

head(tmp)

# sort
tmp1 <- 1:nrow(tmp) # preserve orig order in case of tie
