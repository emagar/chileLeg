...
sel <- which(tmpdat$dateIn >= tmp[2] & tmpdat$dateIn < tmp[3]); tmpdat$dleg98[sel] <- 1; tmpdat$legis[sel] <- 1998
sel <- which(tmpdat$dateIn >= tmp[3] & tmpdat$dateIn < tmp[4]); tmpdat$dleg02[sel] <- 1; tmpdat$legis[sel] <- 2002
sel <- which(tmpdat$dateIn >= tmp[4] & tmpdat$dateIn < tmp[5]); tmpdat$dleg06[sel] <- 1; tmpdat$legis[sel] <- 2006
sel <- which(tmpdat$dateIn >= tmp[5] & tmpdat$dateIn < tmp[6]); tmpdat$dleg10[sel] <- 1; tmpdat$legis[sel] <- 2010
#
...
chainsHda <- allUrg$chains[sel,]
chainsHda[1,]
#
# create DVs
chainsHda$danyReportwiDeadline <- 0 # will receive info
chainsHda$dhdaReportwiDeadline <- 0 # will receive info
for (i in 1:length(target)){ # loop over boletines
    #i <- 1 # debug
    message(sprintf("loop %s of %s", i, length(target)))
    sel <- which(repsHda$bol %in% target[i])
    tmpRep <- repsHda[sel,]
    sel <- which(chainsHda$bol %in% target[i])
    tmpCh <- chainsHda[sel,]
...
chainsHda$d2wk <-    as.numeric(chainsHda$typeN==2 | chainsHda$typeN==2.4 | chainsHda$typeN==2.45 | chainsHda$typeN==2.5)
chainsHda$d4wk <-    as.numeric(chainsHda$typeN==3 | chainsHda$typeN==3.4 | chainsHda$typeN==3.45 | chainsHda$typeN==3.5)
chainsHda$dextend <- as.numeric(chainsHda$typeN==1.4 | chainsHda$typeN==1.45 | chainsHda$typeN==2.4 | chainsHda$typeN==2.45 | chainsHda$typeN==3.4 | chainsHda$typeN==3.45)
chainsHda$dwithdr <- as.numeric(chainsHda$typeN==1.45 | chainsHda$typeN==1.5 | chainsHda$typeN==2.45 | chainsHda$typeN==2.5 | chainsHda$typeN==3.45 | chainsHda$typeN==3.5)
#
chainsHda$dmocion <- 0
chainsHda$pctcon <- 0
chainsHda$pctright <- 0
chainsHda$dom <- "."
for(i in 1:nrow(chainsHda)){
    #i <- 1 # debug
    message(sprintf("loop %s of %s", i, nrow(chainsHda)))
    target <- which(bills$info$bol %in% chainsHda$bol[i])
    chainsHda$dmocion[i] <- 1-bills$info$dmensaje[target]
    chainsHda$pctcon[i] <- bills$info$pctcon[target]
    chainsHda$pctright[i] <- bills$info$pctright[target]
...
chainsHda$dmocionMix[chainsHda$pctpdt==0 | chainsHda$pctpdt==100] <- 0
#
# Add president's maj status in chamber
...
sel <- which(chainsHda$on >= tmp[4] & chainsHda$on < tmp[5]); chainsHda$dmajSen[sel] <- 1 # tie coded as maj for pdt
sel <- which(chainsHda$on >= tmp[5] & chainsHda$on < tmp[6]); chainsHda$dmajSen[sel] <- 0
sel <- which(chainsHda$on >= tmp[6] & chainsHda$on < tmp[7]); chainsHda$dmajSen[sel] <- 1 # tie coded as maj for pdt
sel <- which(chainsHda$on >= tmp[7] & chainsHda$on < tmp[8]); chainsHda$dmajSen[sel] <- 1
sel <- which(chainsHda$on >= tmp[8] & chainsHda$on < tmp[9]); chainsHda$dmajSen[sel] <- 0
#
# dip: always maj=1 (2010-14 50%, coded 1)
chainsHda$dmajDip <- 1
#
...
sel <- which(chainsHda$on >= tmp[2] & chainsHda$on < tmp[3]); chainsHda$pterm[sel] <- round((tmp[3] - chainsHda$on[sel]) * 100 / (365*6), digits = 0)
sel <- which(chainsHda$on >= tmp[3] & chainsHda$on < tmp[4]); chainsHda$pterm[sel] <- round((tmp[4] - chainsHda$on[sel]) * 100 / (365*4), digits = 0)
...
sel <- which(chainsHda$on >= tmp[2] & chainsHda$on < tmp[3]); chainsHda$dterm[sel] <- round((tmp[3] - chainsHda$on[sel]) * 100 / (365*4), digits = 0)
sel <- which(chainsHda$on >= tmp[3] & chainsHda$on < tmp[4]); chainsHda$dterm[sel] <- round((tmp[4] - chainsHda$on[sel]) * 100 / (365*4), digits = 0)
sel <- which(chainsHda$on >= tmp[4] & chainsHda$on < tmp[5]); chainsHda$dterm[sel] <- round((tmp[5] - chainsHda$on[sel]) * 100 / (365*4), digits = 0)
...
sel <- which(chainsHda$on >= tmp[2] & chainsHda$on < tmp[3]); chainsHda$sterm[sel] <- round((tmp[3] - chainsHda$on[sel]) * 100 / (365*8), digits = 0)
...
sel <- which(chainsHda$on >= tmp[2]  & chainsHda$on < tmp[3]);  chainsHda$legyr[sel] <- round((tmp[3]  - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[3]  & chainsHda$on < tmp[4]);  chainsHda$legyr[sel] <- round((tmp[4]  - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[4]  & chainsHda$on < tmp[5]);  chainsHda$legyr[sel] <- round((tmp[5]  - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[5]  & chainsHda$on < tmp[6]);  chainsHda$legyr[sel] <- round((tmp[6]  - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[6]  & chainsHda$on < tmp[7]);  chainsHda$legyr[sel] <- round((tmp[7]  - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[7]  & chainsHda$on < tmp[8]);  chainsHda$legyr[sel] <- round((tmp[8]  - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[8]  & chainsHda$on < tmp[9]);  chainsHda$legyr[sel] <- round((tmp[9]  - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[9]  & chainsHda$on < tmp[10]); chainsHda$legyr[sel] <- round((tmp[10] - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[10] & chainsHda$on < tmp[11]); chainsHda$legyr[sel] <- round((tmp[11] - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[11] & chainsHda$on < tmp[12]); chainsHda$legyr[sel] <- round((tmp[12] - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[12] & chainsHda$on < tmp[13]); chainsHda$legyr[sel] <- round((tmp[13] - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[13] & chainsHda$on < tmp[14]); chainsHda$legyr[sel] <- round((tmp[14] - chainsHda$on[sel]) * 100 / 365, digits = 0)
sel <- which(chainsHda$on >= tmp[14] & chainsHda$on < tmp[15]); chainsHda$legyr[sel] <- round((tmp[15] - chainsHda$on[sel]) * 100 / 365, digits = 0)
...
sel <- which(chainsHda$on >= tmp[1] & chainsHda$on < tmp[2]); chainsHda$dleg94[sel] <- 1; chainsHda$legis[sel] <- 1994
sel <- which(chainsHda$on >= tmp[2] & chainsHda$on < tmp[3]); chainsHda$dleg98[sel] <- 1; chainsHda$legis[sel] <- 1998
sel <- which(chainsHda$on >= tmp[3] & chainsHda$on < tmp[4]); chainsHda$dleg02[sel] <- 1; chainsHda$legis[sel] <- 2002
sel <- which(chainsHda$on >= tmp[4] & chainsHda$on < tmp[5]); chainsHda$dleg06[sel] <- 1; chainsHda$legis[sel] <- 2006
sel <- which(chainsHda$on >= tmp[5] & chainsHda$on < tmp[6]); chainsHda$dleg10[sel] <- 1; chainsHda$legis[sel] <- 2010
#
table(chainsHda$legis) # hay dos cadenas iniciadas el 10-3-1998... ¿cómo se han colado?
#
# bill introduced after ley orgánica relaxed urgency deadlines
...
#
# FIT MODELS
chainsAll$ptermR <- scale(chainsAll$pterm) # rescale/center continuous variables to aid identification of multilevel model
chainsAll$legyrR <- scale(chainsAll$legyr) # rescale/center continuous variables to aid identification of multilevel model
#
fit11 <- glm(danyReportwiDeadline ~ d2wk + d4wk + dextend + dwithdr + dmocion                                    + dmajSen + dsen + ptermR + legyrR + dreform2010                   , data = chainsAll, family = binomial(link = logit))
fit12 <- glm(danyReportwiDeadline ~ d2wk + d4wk + dextend + dwithdr + dmocionAllPdt + dmocionMix + dmocionAllOpp + dmajSen + dsen + ptermR + legyrR + dreform2010, data = chainsAll, family = binomial(link = logit))
fit13 <- glm(danyReportwiDeadline ~ d2wk + d4wk + dextend + dwithdr + dmocionAllPdt + dmocionMix + dmocionAllOpp + dmajSen + dsen + ptermR + legyrR + dreform2010 + as.factor(legis), data = chainsAll, family = binomial(link = logit))
# multilevel version with error terms clustered by legislatura (Gelman Hill p. 302 from eq 12.13)
library(lme4)
fit14 <- glmer(danyReportwiDeadline ~ d2wk + d4wk + dextend + dwithdr +dmocionAllPdt + dmocionMix + dmocionAllOpp  + dmajSen + dsen + ptermR + legyrR + dreform2010 + (1|legis), data = chainsAll, family = binomial(link ="logit"))
#
# use mutually-exclusive dummies for each type of urgency message
fit15 <- glm(danyReportwiDeadline ~ as.factor(typeN)                + dmocion                                    + dmajSen + dsen + pterm + legyr + dreform2010, data = chainsAll, family = binomial(link = logit))
fit16 <- glm(danyReportwiDeadline ~ as.factor(typeN)                + dmocionAllPdt + dmocionMix + dmocionAllOpp + dmajSen + dsen + pterm + legyr + dreform2010, data = chainsAll, family = binomial(link = logit))
#
summary(fit1)
summary(fit2)
summary(fit3)
...
#
pred11 <- predict(fit11, type = "response"); pred11[pred11>=.5] <- 1; pred11[pred11<.5] <- 0
table(chainsAll$danyReportwiDeadline - pred11) / nrow(chainsAll) # pct correctly predicted
pred12 <- predict(fit12, type = "response"); pred12[pred12>=.5] <- 1; pred12[pred12<.5] <- 0
table(chainsAll$danyReportwiDeadline - pred12) / nrow(chainsAll) # pct correctly predicted
pred13 <- predict(fit13, type = "response"); pred13[pred13>=.5] <- 1; pred13[pred13<.5] <- 0
table(chainsAll$danyReportwiDeadline - pred13) / nrow(chainsAll) # pct correctly predicted
pred14 <- predict(fit14, type = "response"); pred14[pred14>=.5] <- 1; pred14[pred14<.5] <- 0
table(chainsAll$danyReportwiDeadline - pred14) / nrow(chainsAll) # pct correctly predicted
# export to latex
library(stargazer)
stargazer(fit1, fit2, fit3, fit4, fit11, fit12, fit13, fit14, title="Regression results", align=TRUE, report = ('vc*p'),
          covariate.labels=
 c("2 week notice",
   "4 week notice",
   "Change deadline",
...
dummy=1 if extended
dummy=1 if cut
dummy=1 if withdrawn
(baseline= no urgency)

model 1: dv=1 if bill reported; dv=1 if bill reported before deadline; dv=1 if bill reported in twice deadline
member bill
hacienda
senmaj
insenate
...
explore calvo's mixture survival
